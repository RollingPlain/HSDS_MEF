...
class HyperOptimizer(object):
  ...
  def __init__(self, model, args):
    ...
    self.model = model
    self.optimizer = torch.optim.Adam(self.model.hyper_parameters(), lr=args.hyper_learning_rate, betas=(0.5, 0.999), weight_decay=args.hyper_weight_decay)
  ...
  def _compute_unrolled_model(self, input, target, eta, network_optimizer):
    ...
    dtheta = _concat(grad_).data + self.network_weight_decay * theta
  ...
  def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
    ...
    unrolled_loss = unrolled_model.calculate_CL(x, target_valid)[0]
